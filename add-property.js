...
async function publishProperty() {
  ...
  try {
    ...
    const saveRes = await fetch(editingProperty ? `http://localhost:3000/properties/${encodeURIComponent(editingProperty.id)}` : "http://localhost:3000/properties", {
    ...
    if (!editingProperty && created && created.id) {
      try {
        await fetch(`http://localhost:3000/properties/${created.id}`, {
